...
public class TournamentGUI extends JPanel {
    ...
    // Labels
    private JLabel scoreLabel, chipsLabel, playerHandValueLabel, dealerHandValueLabel, gameMessageLabel, tournamentTargetLabel; // Added tournamentTargetLabel
    // Card Labels
    ...
    private void setupButtonActionListeners() {
        ...
        backToMenuButton.addActionListener(e -> {
            ...
            if (currentFrame != null) {
                ...
                System.err.println("Error: Could not find parent frame for GameGUI.");
            }
        });
    }
    ...
    private void updateLabels() {
        ...
        gameMessageLabel.setText("<html><body style='text-align: center; width: 95%;'>" + (message != null ? message : "") +
                "</body></html>");
        ...
        if (gameLogic.currentTournament != null) { // Check if tournament is initialized
            tournamentTargetLabel.setText("Target: " + gameLogic.currentTournament.getTargetChips());
        } else {
            tournamentTargetLabel.setText("Target: N/A"); // Or some default if not in tournament
        }
    }
    ...
    private void updateCardDisplay() {
        if (gameLogic.getCurrentState() == TournamentGameLogic.GameState.BETTING) {
            if (dealerCardLabels[0] != null) {
                dealerCardLabels[0].setIcon(cardDownIcon);
                dealerCardLabels[0].setVisible(true);
            }
            if (dealerCardLabels[1] != null) {
                dealerCardLabels[1].setIcon(cardDownIcon);
                dealerCardLabels[1].setVisible(true);
            }
            if (playerCardsLabels[0] != null) {
                playerCardsLabels[0].setIcon(cardDownIcon);
                playerCardsLabels[0].setVisible(true);
            }
            if (playerCardsLabels[1] != null) {
                playerCardsLabels[1].setIcon(cardDownIcon);
                playerCardsLabels[1].setVisible(true);
            }
            ...
            updateSpecificHandDisplay(playerCardsLabels, gameLogic.getPlayerCards(), false);
        }
    }
    ...
    private ImageIcon getCardImage(String cardString) {
        ...
        try {
            ...
            String rankShort = rankName.equals("Ten") ? "T" : (rankName.length() > 0 ? rankName.substring(0, 1) : "?");
            String suitShort = suitName.length() > 0 ? suitName.substring(0, 1) : "?";
    ...
    private void updateButtonStates() {
        ...
        backToMenuButton.setVisible(currentState == TournamentGameLogic.GameState.ROUND_OVER || currentState == TournamentGameLogic.GameState.GAME_OVER);
        ...
        nextRoundButton.setEnabled(nextRoundButton.isVisible());
        restartButton.setEnabled(restartButton.isVisible());
        exitButton.setEnabled(exitButton.isVisible());
        backToMenuButton.setEnabled(backToMenuButton.isVisible());
